...
def find_neighbors(pindex, triang):
    ...
    for simplex in triang.vertices:
        if pindex in simplex:
            neighbors.extend(
                [simplex[i] for i in range(len(simplex)) if simplex[i] != pindex]
            )
...
def bond_angle_order(lattice, nearest_neighbors=6):
    if (
        len(lattice) < nearest_neighbors + 1
    ):  # need at least a point and its neighbors :)
        return 0
    nnAngl = []
    order_contrib = []
    ...
    for k in range(len(lattice)):
        if (
            lattice[k][0] == max_x
            or lattice[k][0] == min_x
            or lattice[k][1] == max_y
            or lattice[k][1] == min_y
        ):
            boundary_cells += 1
            continue
