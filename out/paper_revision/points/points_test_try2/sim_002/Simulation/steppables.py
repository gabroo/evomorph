...
class TwoLayer(SteppableBasePy):
    def __init__(self, params, d_out, frequency=1):
        ...
        self.c.execute("create table points (t, type, value)")
    ...
    def step(self, mcs):
        ...
        # tracking stuff for homogeneity (psi)
        NBR = 0
        NYG = 0
        CSABR = 0
        CSAYG = 0
        pts = {tp: 0 for tp in self.cell_types}
        ...
        for cell in self.cellList:  # iterate over cell list
            ...
            if cell.type == Type.YELLOW or cell.type == Type.GREEN:  # YG
                DTRES = (
                    1 / (ALPHAYG + math.exp(-((PTSB + PTSR) - BETAYG) / EPSILONYG))
                ) - (1 / KAPPAYG) * cell.dict["pts"]
                cell.dict["pts"] += DTRES
                hom_yg = csas[Type.GREEN] + csas[Type.YELLOW]
                if hom_yg:
                    CSAYG += (hom_yg) / cell.surface
                    NYG += 1

            if cell.type == Type.BLUE or cell.type == Type.RED:  # BR
                hom_br = csas[Type.RED] + csas[Type.BLUE]
                if hom_br:
                    CSABR += (hom_br) / cell.surface
                    NBR += 1
        ...
        if not mcs % RESOL:
            inserts = [(mcs, t, pts[t]) for t in self.cell_types]
            self.c.executemany(
                f"insert into points (t, type, value) values (?, ?, ?)", inserts
            )
